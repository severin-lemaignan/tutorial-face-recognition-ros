...
def pca(X, y, num_components=0):
    ...
    if (num_components <= 0) or (num_components>n):
        num_components = n
    mu = X.mean(axis=0)
    #subplot(title="Debug", images=[X[0,0:92*112].reshape((112,-1)), (X-mu)[0,0:92*112].reshape((112,-1))], rows=2, cols=1,sptitles=["Original", "Original minus mean of all other images"])
    X = X - mu
    if n>d:
        C = np.dot(X.T,X)
